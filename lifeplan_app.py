...
def fund_growth(params, fund, age, cached_returns=None):
...
def apply_pension_market_shocks(params, fund, rita_pool, age, cached_calendar=None):
    """
    ...
    cal = cached_calendar if cached_calendar is not None else deterministic_market_event_calendar(params)
    notes = []
...
def simulate(params, scenario):
    ...
    # Cache di mercato per questa singola simulazione: evita di ricalcolare
    # CAGR/calendario decine di volte per ogni anno.
    _market_calibration_cache = calibrated_normal_returns(params)
    _market_calendar_cache = deterministic_market_event_calendar(params)

    for age in range(age0, age_end + 1):
        y = age - age0
        events = []
        market_eq_return = _market_calibration_cache["equity_normal"]
        market_bd_return = _market_calibration_cache["bond_normal"]
        ...
        if params.get("has_pension_fund", True) and not fund_settled:
            fund = fund_growth(params, fund, age, (market_eq_return, market_bd_return))
            if rita_pool > 0:
                rita_pool = fund_growth(params, rita_pool, age, (market_eq_return, market_bd_return))

            fund, rita_pool, pension_market_notes = apply_pension_market_shocks(
                params, fund, rita_pool, age, _market_calendar_cache
            )
        ...
        market_calendar = _market_calendar_cache
...
def mc_evaluate(base, scenario, runs=120, seed=360):
    """
    Valuta una strategia sugli stessi futuri esogeni determinati dal seed.
    Successo = nessuna vera insolvenza + patrimonio finale >= vincolo.
    Il normale decumulo NON è insolvenza.
    """
    ...
    for _ in range(int(runs)):
        ...
        uncovered = df["Fabbisogno non coperto"] > 1e-9
        purchase_uncovered = df["Fabbisogno non coperto acquisto casa"] > 1e-9
        insolvent=bool(uncovered.any() or purchase_uncovered.any())

        if insolvent:
            insolvencies += 1
            ages=[]
            if uncovered.any():
                ages.append(int(df.loc[uncovered,"Età"].iloc[0]))
            if purchase_uncovered.any():
                ages.append(int(df.loc[purchase_uncovered,"Età"].iloc[0]))
            if ages:
                insolvency_ages.append(min(ages))

        target_fail = final < float(base["target_final"])
        if target_fail and not insolvent:
            target_only_failures += 1

        if bool(df["Vendita azionario durante cigno nero"].any()):
            forced_crisis_sales += 1

        if insolvent or target_fail:
            failures += 1
    ...
    return {
        ...
        "insolvency_rate":insolvencies/n,
        "target_only_failure_rate":target_only_failures/n,
        "crisis_sale_rate":forced_crisis_sales/n,
        "p5":float(np.percentile(a,5)),
        "p10":float(np.percentile(a,10)),
        "p50":float(np.percentile(a,50)),
        "p90":float(np.percentile(a,90)),
        "median_insolvency_age":float(np.median(insolvency_ages)) if insolvency_ages else np.nan,
    }
...
if "lifeplan_run_params" in st.session_state:
    ...
    st.caption(
        "Confronto equo: tutte le età e tutti i portafogli candidati vengono testati sugli "
        "stessi identici futuri Monte Carlo (common random numbers). Un normale decumulo "
        "non è insolvenza: insolvenza significa avere ancora un fabbisogno dopo aver esaurito "
        "tutte le risorse finanziarie disponibili."
    )

    with st.expander("ℹ️ Come leggere gli output Monte Carlo", expanded=False):
        st.markdown("""
**Quando una simulazione è un successo?**

Una singola traiettoria Monte Carlo è considerata **positiva** soltanto se soddisfa entrambe le condizioni:

1. **Nessuna insolvenza a qualsiasi età** della simulazione.
2. Alla fine della simulazione il **patrimonio finanziario residuo è almeno pari al patrimonio minimo desiderato** impostato dall'utente.

Il normale **decumulo del patrimonio non è insolvenza**. L'insolvenza si verifica soltanto quando,
dopo aver utilizzato le risorse finanziarie disponibili secondo le regole del modello, rimane ancora
un fabbisogno economico non coperto.

**Successo MC** indica quindi la percentuale di traiettorie che superano entrambi i test.

**Insolvenza vera** indica la percentuale di traiettorie nelle quali le risorse finanziarie diventano
insufficienti a coprire le uscite almeno una volta durante la vita simulata.

**Solo vincolo finale fallito** indica la percentuale di traiettorie che non diventano mai insolventi,
ma terminano con un patrimonio finanziario inferiore al patrimonio minimo desiderato a fine simulazione.

Il **patrimonio finanziario minimo di sicurezza** è invece una soglia di attenzione durante il percorso:
scendere temporaneamente sotto tale valore non equivale automaticamente a fallire la simulazione.

---

**P5, P10, P50 e P90**

Sono percentili del **patrimonio finanziario finale** ottenuto dalle diverse traiettorie Monte Carlo:

- **P5** — il 5% dei risultati termina sotto questo valore e il 95% sopra. Rappresenta una coda molto sfavorevole.
- **P10** — il 10% termina sotto questo valore e il 90% sopra. È un indicatore prudenziale importante.
- **P50** — è la **mediana**: il 50% delle simulazioni termina sotto questo valore e il 50% sopra. Rappresenta il risultato centrale, non una garanzia.
- **P90** — il 90% termina sotto questo valore e il 10% sopra. Rappresenta una coda particolarmente favorevole.

Per valutare un'età di pensionamento non bisogna quindi guardare soltanto il P50. Sono particolarmente
importanti **Successo MC, Insolvenza vera, P5/P10 e il rispetto del patrimonio finale minimo**.
""")

        st.info(
            "Esempio: Successo MC 87% può significare 8% di traiettorie in vera insolvenza "
            "+ 5% che arrivano alla fine senza insolvenza ma sotto il patrimonio finale minimo. "
            "Il restante 87% soddisfa entrambi i requisiti."
        )
    ...
    with st.expander("1 · Ottimizza Pensione", expanded=True):
        ...
        if st.button("🎯 Ottimizza età pensionamento", use_container_width=True):
            ...
            for j,age in enumerate(ages):
                ...
                m=mc_evaluate(pp,opt_scenario,mc_runs,seed=360)
                rows_opt.append({
                    "Età uscita":age,
                    "Successo MC":m["success_rate"],
                    "P5 finale":m["p5"],
                    "P10 finale":m["p10"],
                    "P50 finale":m["p50"],
                    "P90 finale":m["p90"],
                    "Insolvenza vera":m["insolvency_rate"],
                    "Solo vincolo finale fallito":m["target_only_failure_rate"],
                    "Età mediana insolvenza":m["median_insolvency_age"],
            ...
            st.dataframe(odf.style.format({
                ...
                "P90 finale":"€{:,.0f}",
                "Insolvenza vera":"{:.1%}",
                "Solo vincolo finale fallito":"{:.1%}",
                "Età mediana insolvenza":"{:.0f}",
                "Vendita azioni in cigno nero":"{:.1%}"
            }),use_container_width=True,hide_index=True)

    with st.expander("2 · Ottimizza Stile di Vita"):
        st.write(
            "**Domanda:** fissata l'età di uscita, qual è la massima spesa sostenibile? "
            "Le spese Senior/Old/Older vengono scalate proporzionalmente; il portafoglio resta invariato."
        )
        life_ages=[]
        cols=st.columns(5)
        ...
        if st.button("💶 Ottimizza stile di vita", use_container_width=True):
            with st.status("Ottimizzazione stile di vita in corso...", expanded=True) as status:
                out_rows=[]
                # Ricerca rapida: 6 iterazioni; verifica finale con il numero MC scelto.
                search_runs=max(20,min(60,int(mc_runs)//2))
                for j,age in enumerate(life_ages):
                    st.write(f"Test pensione a {int(age)} anni…")
                    lo,hi=0.40,2.50
                    best=0.0
                    for step in range(6):
                        mid=(lo+hi)/2
                        pp=scale_expenses(params,mid); pp["retire_age"]=int(age)
                        m=mc_evaluate(pp,opt_scenario,search_runs,seed=7200)
                        if m["success_rate"]>=success_threshold:
                            best=mid; lo=mid
                        else:
                            hi=mid
                    if best>0:
                        pp=scale_expenses(params,best); pp["retire_age"]=int(age)
                        bestm=mc_evaluate(pp,opt_scenario,int(mc_runs),seed=87200)
                        feasible=bestm["success_rate"]>=success_threshold
                    else:
                        bestm=None; feasible=False

                    base_month=sum(params["expense_bands"]["Senior"].values())
                    out_rows.append({
                        "Età uscita":int(age),
                        "Fattibile":feasible,
                        "Moltiplicatore spese":best if feasible else np.nan,
                        "Spesa Senior max €/mese oggi":base_month*best if feasible else np.nan,
                        "Successo MC":bestm["success_rate"] if bestm else 0.0,
                        "P10 finale":bestm["p10"] if bestm else np.nan,
                        "P50 finale":bestm["p50"] if bestm else np.nan,
                    })
                st.session_state["lifeplan_style_optimizer"] = pd.DataFrame(out_rows)
                status.update(label="Ottimizzazione stile di vita completata", state="complete")

        if "lifeplan_style_optimizer" in st.session_state:
            ldf=st.session_state["lifeplan_style_optimizer"]
            st.success("Risultati ultima ottimizzazione stile di vita")
            st.dataframe(ldf.style.format({
                "Moltiplicatore spese":"{:.2f}x",
                "Spesa Senior max €/mese oggi":"€{:,.0f}",
                "Successo MC":"{:.1%}",
                "P10 finale":"€{:,.0f}",
                "P50 finale":"€{:,.0f}",
            }),use_container_width=True,hide_index=True)
    ...
    with st.expander("3 · Ottimizza Portafoglio"):
        st.write(
            "**Obiettivo:** trovare la strategia con la **minore esposizione azionaria** che "
            "raggiunge il patrimonio finale vincolo con la probabilità Monte Carlo richiesta. "
            "Non massimizza il patrimonio."
        )
        port_ages=[]
        cols=st.columns(5)
        ...
        if st.button("🛡️ Ottimizza portafoglio minimo-rischio", type="primary", use_container_width=True):
            with st.status("Ottimizzazione portafoglio in corso...", expanded=True) as status:
                port_rows=[]
                chosen={}
                search_runs=max(20,min(60,int(mc_runs)//2))
                for j,age in enumerate(port_ages):
                    st.write(f"Ricerca portafoglio minimo-rischio per pensione a {int(age)} anni…")
                    winner=None; winner_m=None
                    candidates=portfolio_candidates(params,int(age))
                    for ci,cand in enumerate(candidates):
                        # screening economico
                        m0=mc_evaluate(cand,opt_scenario,search_runs,seed=99000)
                        if m0["success_rate"]>=max(0.0,success_threshold-0.03):
                            # verifica robusta solo sui candidati promettenti
                            m=mc_evaluate(cand,opt_scenario,int(mc_runs),seed=199000)
                            if m["success_rate"]>=success_threshold:
                    ...
                    if winner is None:
                        port_rows.append({
                            "Età uscita":int(age),"Fattibile":False,
                        ...
                        port_rows.append({
                            "Età uscita":int(age),"Fattibile":True,
                            "Azionario medio vita":portfolio_risk_score(winner),
                            "Allocazione iniziale":f"{a[0]*100:.0f}% az. / {a[1]*100:.0f}% obbl. / {a[2]*100:.0f}% liq.",
                            "Successo MC":winner_m["success_rate"],
                            "P10 finale":winner_m["p10"],
                            "P50 finale":winner_m["p50"],
                            "Vendita azioni in cigno nero":winner_m["crisis_sale_rate"]
                        })

                st.session_state["lifeplan_port_optimizer"] = pd.DataFrame(port_rows)
                st.session_state["lifeplan_port_chosen"] = chosen
                status.update(label="Ottimizzazione portafoglio completata", state="complete")

        if "lifeplan_port_optimizer" in st.session_state:
            pdf=st.session_state["lifeplan_port_optimizer"]
            st.success("Risultati ultima ottimizzazione portafoglio")
            st.dataframe(pdf.style.format({
                "Azionario medio vita":"{:.1%}",
                "Successo MC":"{:.1%}",
                "P10 finale":"€{:,.0f}",
                "P50 finale":"€{:,.0f}",
                "Vendita azioni in cigno nero":"{:.1%}",
            ...
            chosen=st.session_state.get("lifeplan_port_chosen",{})
            if chosen:
                age_show=st.selectbox(
                    "Mostra glide path ottimizzato per età",
                    sorted(chosen.keys()),
                    key="show_port_age"
                )
                win=chosen[age_show]
                gl=[{
                    "Stadio":"Senior iniziale","Età":int(win["age_start"]),
                    "Azionario":win["alloc_initial"][0],
                    "Obbligazioni":win["alloc_initial"][1],
                    "Liquidità":win["alloc_initial"][2]
                }]
                for stg in win["allocation_stages"]:
                    gl.append({
                        "Stadio":stg["name"],"Età":stg["age"],
                        "Azionario":stg["alloc"][0],
                        "Obbligazioni":stg["alloc"][1],
                        "Liquidità":stg["alloc"][2]
                    })
                gdf=pd.DataFrame(gl)
                st.dataframe(
                    gdf.style.format({
