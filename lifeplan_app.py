...
def calibrated_normal_returns(params):
    """Calibra i rendimenti degli anni normali per rispettare il CAGR target complessivo."""
    ...
    for age in ages:
        for e in cal["equity_crises"]:
            if e.get("active", True) and e["age"] <= age < e["age"] + max(1,e.get("duration",3)):
                eq_event_years.add(age)
                decline = max(1, int(math.ceil(max(1,e.get("duration",3))/3)))
                if age < e["age"] + decline:
                    dd = float(e.get("drawdown",0.25))
                    annual_drop = 1 - (1-dd)**(1/decline)
                    eq_factor *= (1-annual_drop)
                else:
                    eq_factor *= (1+float(e.get("recovery_return",0.10)))

        for e in cal["systemic_crises"]:
            if e.get("active", True) and e["age"] <= age < e["age"] + max(1,e.get("duration",4)):
                eq_event_years.add(age)
                bd_event_years.add(age)
                decline = max(1, int(math.ceil(max(1,e.get("duration",4))/3)))
                if age < e["age"] + decline:
                    eq_dd = float(e.get("equity_drawdown",0.35))
                    bd_dd = float(e.get("bond_drawdown",0.08))
                    eq_drop = 1 - (1-eq_dd)**(1/decline)
                    bd_drop = 1 - (1-bd_dd)**(1/decline)
                    eq_factor *= (1-eq_drop)
                    bd_factor *= (1-bd_drop)
                else:
                    eq_factor *= (1+float(e.get("equity_recovery_return",0.10)))
                    bd_factor *= (1+float(e.get("bond_recovery_return",0.04)))

    eq_normal_years = max(1, n-len(eq_event_years))
    bd_normal_years = max(1, n-len(bd_event_years))

    eq_goal = (1+targets["equity"])**n
    bd_goal = (1+targets["bonds"])**n

    eq_normal = (eq_goal/max(eq_factor,1e-12))**(1/eq_normal_years)-1
    bd_normal = (bd_goal/max(bd_factor,1e-12))**(1/bd_normal_years)-1

    return {
        "equity_target": targets["equity"],
        "bond_target": targets["bonds"],
        "equity_normal": eq_normal,
        "bond_normal": bd_normal,
        "equity_event_years": eq_event_years,
        "bond_event_years": bd_event_years,
    }
...
def market_return_for_age(params, age):
    ...
    return (
        0.0 if age in c["equity_event_years"] else c["equity_normal"],
        0.0 if age in c["bond_event_years"] else c["bond_normal"],
    )

def deterministic_market_event_calendar(params):
    mode=params.get("market_scenario","Mediano")
    sequence=params.get("market_sequence","Centrale")
    start=int(params["age_start"]); end=int(params["age_end"]); horizon=max(1,end-start+1)
    ...
    if mode=="Ottimista":
        eq_count=max(1,round(horizon/13)); sys_count=0 if horizon<35 else 1
        eq_dd,eq_rec=0.22,0.11
        sys_eq_dd,sys_bd_dd,sys_eq_rec,sys_bd_rec=0.28,0.05,0.12,0.05
    elif mode=="Pessimista":
        eq_count=max(2,round(horizon/5.5)); sys_count=1 if horizon<30 else 2 if horizon<55 else 3
        eq_dd,eq_rec=0.32,0.08
        sys_eq_dd,sys_bd_dd,sys_eq_rec,sys_bd_rec=0.45,0.12,0.09,0.04
    else:
        eq_count=max(1,round(horizon/7)); sys_count=0 if horizon<25 else 1 if horizon<50 else 2
        eq_dd,eq_rec=0.27,0.10
        sys_eq_dd,sys_bd_dd,sys_eq_rec,sys_bd_rec=0.36,0.08,0.10,0.045

    retire=int(params["retire_age"])
    def ages(count,lo,hi):
        if count<=0: return []
        lo=int(lo); hi=int(max(lo,hi))
        if count==1: return [int(round((lo+hi)/2))]
        return [int(round(lo+(i+1)/(count+1)*(hi-lo))) for i in range(count)]

    if sequence=="Favorevole":
        eq_ages=ages(eq_count,start+2,max(start+3,min(retire-2,end-2)))
        sys_ages=ages(sys_count,start+4,max(start+5,min(retire-3,end-4)))
    elif sequence=="Sfavorevole":
        shock_start=max(start+2,retire-2)
        eq_ages=ages(eq_count,shock_start,end-2)
        sys_ages=ages(sys_count,max(shock_start,retire),end-3)
    else:
        eq_ages=ages(eq_count,start+3,end-3)
        sys_ages=ages(sys_count,start+6,end-5)

    sys_set=set(sys_ages); cleaned=[]
    for a in eq_ages:
        while a in sys_set and a<end-1: a+=1
        cleaned.append(a)

    return {
        "equity_crises":[{"active":True,"age":a,"duration":3,"drawdown":eq_dd,"recovery_return":eq_rec} for a in cleaned],
        "systemic_crises":[{"active":True,"age":a,"duration":4,"equity_drawdown":sys_eq_dd,"bond_drawdown":sys_bd_dd,"equity_recovery_return":sys_eq_rec,"bond_recovery_return":sys_bd_rec} for a in sys_ages],
    }


def apply_equity_only_crisis(equity, age, event):
    if not event.get("active",True): return equity,""
    start=int(event["age"]); dur=max(1,int(event.get("duration",3)))
    if not(start<=age<start+dur): return equity,""
    decline=max(1,int(np.ceil(dur/3)))
    if age<start+decline:
        dd=float(event.get("drawdown",0.25)); drop=1-(1-dd)**(1/decline)
        return equity*(1-drop),f"Crisi azionaria: -{drop*100:.1f}% azionario"
...
def apply_systemic_crisis(equity,bonds,age,event):
    if not event.get("active",True): return equity,bonds,""
    start=int(event["age"]); dur=max(1,int(event.get("duration",4)))
    if not(start<=age<start+dur): return equity,bonds,""
    decline=max(1,int(np.ceil(dur/3)))
    if age<start+decline:
        edd=float(event.get("equity_drawdown",0.35)); bdd=float(event.get("bond_drawdown",0.08))
        edrop=1-(1-edd)**(1/decline); bdrop=1-(1-bdd)**(1/decline)
        return equity*(1-edrop),bonds*(1-bdrop),f"Cigno nero sistemico: azionario -{edrop*100:.1f}% · obbligazionario -{bdrop*100:.1f}%"
    er=float(event.get("equity_recovery_return",0.10)); br=float(event.get("bond_recovery_return",0.04))
    return equity*(1+er),bonds*(1+br),f"Cigno nero sistemico: recupero azionario +{er*100:.1f}% · obbligazionario +{br*100:.1f}%"
...
with st.sidebar:
    ...
    st.caption("Se gli stadi successivi sono disattivati, questo profilo resta valido fino a fine simulazione.")
    ...
    owned_home_value = st.number_input(
        "Valore casa già posseduta / da acquistare (€)",
        0.0,
    ...
    owned_home_financing = st.radio(
        ...
        disabled=not owns_home_initially,
    )
    owned_mortgage_payment = st.number_input(
        "Rata mensile mutuo residuo (€)", 0.0,
        value=float(gv("owned_mortgage_payment", 900.0)), step=50.0,
        disabled=(not owns_home_initially or owned_home_financing != "Con mutuo in corso"),
    )
    owned_mortgage_months_left = st.number_input(
        "Mensilità mancanti", 0, 600,
    ...
    owned_home_maintenance_pct = st.number_input(
        ...
        0.1,
        help="Guida: 0,5% bassa · 1,0% centrale · 1,5% alta · 2,0%+ per immobili vecchi/manutenzione intensa.",
    ) / 100
    owned_home_ordinary_annual = owned_home_value * owned_home_maintenance_pct
    st.caption(
        f"Valore assoluto: €{owned_home_ordinary_annual:,.0f}/anno "
        f"(€{owned_home_ordinary_annual/12:,.0f}/mese)."
    )
    owned_home_extra_pct = st.number_input(
        ...
        0.1,
        help="Guida come accantonamento medio annualizzato: circa 0,3–0,8% del valore casa; aumentare per immobili datati o interventi prevedibili.",
    ) / 100
    owned_home_extra_annual = owned_home_value * owned_home_extra_pct
    st.caption(
        f"Valore assoluto straordinaria: €{owned_home_extra_annual:,.0f}/anno "
        f"(media €{owned_home_extra_annual/12:,.0f}/mese)."
    )
    ...
    st.caption(
        "Guida crescita affitto: 1–2% prudente · 2–3% centrale · "
        "3–4% pressione elevata · oltre 4% scenario di stress."
    )
    rent_10y = rent_monthly * ((1 + rent_growth) ** 10)
    st.caption(
        f"Con questi input: €{rent_monthly:,.0f}/mese oggi → "
        f"circa €{rent_10y:,.0f}/mese tra 10 anni."
    )
...
if market_calibration["equity_normal"] > 0.15 or market_calibration["bond_normal"] > 0.08:
    st.warning(
        "La combinazione CAGR/shock richiede rendimenti molto elevati negli anni normali: "
