...
def validate_allocation(name, alloc):
    ...
    if abs(total - 1.0) > 0.001:
        st.error(f"{name}: le percentuali devono sommare 100% (ora {total*100:.0f}%).")
        return False
    return True
...
def simulate(params, scenario):
    ...
    if scenario["housing"] == "buy":
        price = params["house_price"]
        down = price * scenario["down_pct"]
        upfront = down + params["buying_costs"]

        use = min(cash, upfront)
        cash -= use
        upfront -= use

        use = min(bonds, upfront)
        bonds -= use
        upfront -= use

        equity -= upfront
        house = price
        mortgage_principal = price - down
        debt = mortgage_principal
        mortgage_monthly = annuity_payment(
            mortgage_principal, params["mortgage_rate"], params["mortgage_years"]
        )
    ...
    for age in range(age0, age_end + 1):
        ...
        if scenario["housing"] == "rent":
            ...
            mortgage_annual = (
                mortgage_monthly * 12
                if elapsed < params["mortgage_years"]
                else 0.0
            )
            housing_cost = mortgage_annual + params["owner_cost_monthly"] * 12 * (
                (1 + params["inflation"]) ** y
            )
...
def summarize(df, params, label):
    ...
    return {
        ...
        "Sostenibile": (
            "Sì"
            if min_money >= 0 and final_money >= params["target_final"]
            else "No"
        ),
    }
...
with st.sidebar:
    ...
    ai_eq = st.slider("Azionario iniziale %", 0, 100, int(saved_alloc[0] * 100)) / 100
    ai_bd = st.slider("Governativi iniziali %", 0, 100, int(saved_alloc[1] * 100)) / 100
    ai_ca = st.slider("Liquidità iniziale %", 0, 100, int(saved_alloc[2] * 100)) / 100
    ...
    d1_eq = st.slider("Azionario dopo derisking 1 %", 0, 100, int(d1_saved[0] * 100)) / 100
    d1_bd = st.slider("Governativi dopo derisking 1 %", 0, 100, int(d1_saved[1] * 100)) / 100
    d1_ca = st.slider("Liquidità dopo derisking 1 %", 0, 100, int(d1_saved[2] * 100)) / 100
    ...
    d2_eq = st.slider("Azionario dopo derisking 2 %", 0, 100, int(d2_saved[0] * 100)) / 100
    d2_bd = st.slider("Governativi dopo derisking 2 %", 0, 100, int(d2_saved[1] * 100)) / 100
    d2_ca = st.slider("Liquidità dopo derisking 2 %", 0, 100, int(d2_saved[2] * 100)) / 100
...
if valid:
    scenarios = [
        ...
        {"label": "Mutuo 20%", "housing": "buy", "down_pct": 0.20, "lifecycle": "keep"},
        {"label": "Mutuo 25%", "housing": "buy", "down_pct": 0.25, "lifecycle": "keep"},
        {"label": "Mutuo 30%", "housing": "buy", "down_pct": 0.30, "lifecycle": "keep"},
        {"label": "Mutuo 25% + Nuda", "housing": "buy", "down_pct": 0.25, "lifecycle": "nuda"},
        {"label": "Mutuo 25% + Downsizing", "housing": "buy", "down_pct": 0.25, "lifecycle": "downsize"},
    ]
